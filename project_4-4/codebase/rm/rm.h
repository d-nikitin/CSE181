...
#define INDEXES_COL_ATTR             "attr"
#define INDEXES_COL_FILE_NAME        "file-name" 
#define INDEXES_COL_ATTR_SIZE        50
#define INDEXES_COL_FILE_NAME_SIZE   50
// 1 null byte, 1 int (id), 2 varchars
#define INDEXES_RECORD_DATA_SIZE 1 + INT_SIZE + INDEXES_COL_ATTR_SIZE + INDEXES_COL_FILE_NAME_SIZE
...
class RM_IndexScanIterator {
 public:
  ...
  RC getNextEntry(RID &rid, void *key) {ix_si.getNextEntry(rid, key);};  	// Get next matching entry
  RC close() {ix_si.close();};             			// Terminate index scan
};
...
private:
  ...
  void prepareIndexesRecordData(int32_t id, const string &tableName, const string &attrName, void *data);
  ...
  // Given table ID, attr name, and table name, creates entry in Index table
  RC insertIndex(int32_t id, const string &tableName, const string &attrName);
